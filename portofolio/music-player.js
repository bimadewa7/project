...
(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      /* Keep the player on its own layer. Do NOT use filter/drop-shadow on
         this root — it shares paint with the sticky header (same z-index)
         and Chromium then draws a magenta fringe around header + player. */
      #musicBoxRoot {
        isolation: isolate;
        filter: none;
  ...
  function injectWidgetHTML() {
    ...
    root.className = 'fixed bottom-6 right-6 z-[80] select-none font-sans text-foreground';
  ...
  function setupSeamlessNavigation() {
    document.addEventListener('click', (e) => {
      ...
      e.preventDefault();
      // SPA preventDefault leaves :focus on the nav link; blur so Chrome
      // does not keep a system-accent glow on header chrome after click.
  ...
  async function loadPageSeamlessly(url, pushToHistory = true) {
    try {
      ...
      if (newMain && currentMain) {
        ...
        // Inject new styles BEFORE removing old ones (prevents flash-of-white)
        ...
        // Now safely remove old styles (new ones already active)
        ...
        // Replace content and attributes directly
  ...
  function updateActiveNavLinks(url) {
    ...
    if (desktopNav) {
      ...
      desktopLinks.forEach(a => {
        ...
        if (isTarget) {
          a.className = 'group relative text-sm font-medium text-[#F2F2F0] py-1';
        } else {
          a.className = 'group relative text-sm text-[#9A9A9E] transition-colors hover:text-[#F2F2F0] py-1';
        }
      });
    }
