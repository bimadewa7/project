...
(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      /* Suppress outline/glow and tap-highlight around navigation */
      header, header * {
  ...
  async function loadPageSeamlessly(url, pushToHistory = true) {
    try {
      ...
      if (newMain && currentMain) {
        ...
        // 2b. Swap page-specific styles in head (keep musicBoxStyles and external links)
        const currentCustomStyles = document.querySelectorAll('head style:not(#musicBoxStyles)');
        currentCustomStyles.forEach(s => s.remove());
  ...
  function updateActiveNavLinks(url) {
    const rawFile = (url || window.location.href).split('/').pop().split('?')[0].split('#')[0] || 'bio.html';
    const currentTarget = (rawFile === 'index.html' || rawFile === '') ? 'bio.html' : rawFile;
    ...
    if (desktopNav) {
      ...
      desktopLinks.forEach(a => {
        ...
        if (!href) return;
        const linkTarget = href.split('/').pop().split('?')[0].split('#')[0] || 'bio.html';
        const isTarget = linkTarget === currentTarget || (linkTarget === 'index.html' && currentTarget === 'bio.html');
