...
(function () {
  ...
  async function loadPageSeamlessly(url, pushToHistory = true) {
    // Lock the music widget for the duration of the transition so the
    // upcoming reflow (innerHTML swap + scroll) can't make the browser
    // spuriously re-evaluate :hover on it (the cause of the flash).
    ...
    try {
      ...
      // 5. Scroll to top.
      // Using 'smooth' here used to animate the scroll over many frames
      // immediately after the large <main> innerHTML swap — two reflow
      // sources overlapping is what produced the stray :hover flash on the
      // fixed music widget. An instant scroll collapses this to one reflow.
      ...
      // Always release the lock, even if the fetch failed above.
      if (musicBoxRoot) {
        // Small delay so we release the lock only after layout has settled,
        // not mid-reflow.
